...
class StoreCamioneroRequest extends FormRequest
{
    ...
    {
        return [
            'documento' => ['required', 'string', 'max:10', 'min:5', 'unique:camioneros,documento', 'regex:/^[0-9]+$/'],
            'nombre' => ['required', 'string', 'max:45', 'min:2', 'regex:/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/'],
            'apellido' => ['required', 'string', 'max:45', 'min:2', 'regex:/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/'],
            'fecha_nacimiento' => ['required', 'date', 'before:today', 'before:-18 years'],
            'licencia' => ['required', 'string', 'max:10', 'min:5', 'unique:camioneros,licencia', 'regex:/^[A-Z0-9]+$/'],
            'telefono' => ['required', 'string', 'max:15', 'min:7', 'regex:/^[0-9\s\-\+\(\)]+$/'],
